...
/// CPU features detected at runtime for optimal algorithm selection
...
pub struct CpuFeatures {
    ...
    pub has_neon: bool,
    /// Hardware CRC32 instruction available
    pub has_crc32: bool,
    ...
    pub fn detect() -> Self {
        #[cfg(target_arch = "x86_64")]
        {
            Self {
                ...
                has_neon: false,
                has_crc32: std::arch::is_x86_feature_detected!("sse4.2"),
                l2_cache_size: detect_l2_cache_x86(),
...
fn detect_l2_cache_x86() -> usize {
    ...
    unsafe {
        ...
        if cpuid.eax >= 0x80000006 {
            let cache_info = __cpuid(0x80000006);
            // ECX bits 16-31 contain L2 cache size in KB
            let l2_kb = (cache_info.ecx >> 16) & 0xFFFF;
            if l2_kb > 0 {
                return (l2_kb as usize) * 1024;
            }
        }
    }
